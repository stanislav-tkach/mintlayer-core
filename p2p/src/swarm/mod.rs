    ...
    <<T as NetworkingService>::Address as FromStr>::Err: Debug,
{
    ...
    fn peer_discovered(&mut self, _peers: &[net::types::AddrInfo<T>]) {
        // self.peer_db.discover_peers(peers);
    ...
    fn peer_expired(&mut self, peers: &[net::types::AddrInfo<T>]) {
        self.peer_db._expire_peers(peers)
    }
    ...
    fn validate_address(&self, address: &T::Address) -> bool {
        self.peer_db.is_address_banned(address)
    }
    ...
    fn validate_peer_id(&self, peer_id: &T::PeerId) -> bool {
        self.peer_db.is_id_banned(peer_id)
    }
    ...
    ///
    /// TODO: create generic versions of these
    ///
    /// - `/meshsub/1.1.0`
    /// - `/meshsub/1.0.0`
    /// - `/ipfs/ping/1.0.0`
    ...
    fn validate_supported_protocols(&self, _protocols: &[T::ProtocolId]) -> bool {
        true // TODO: implement
    }
    ...
    /// is checked and updated at least once every 30 seconds. In high-traffic scenarios it
    /// may be beneficial clamp to value to some sensible upper bound.
    ///
