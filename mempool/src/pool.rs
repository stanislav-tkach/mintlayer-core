...
const ROLLING_FEE_DECAY_INTERVAL: Time = 10;
...
trait GetParents {
    fn get_parents(&self, store: &MempoolStore) -> BTreeSet<H256>;
}

impl GetParents for &TxMempoolEntry {
    fn get_parents(&self, _store: &MempoolStore) -> BTreeSet<H256> {
        self.parents.to_owned()
    }
}

impl GetParents for Transaction {
    fn get_parents(&self, store: &MempoolStore) -> BTreeSet<H256> {
        self.get_inputs()
            .iter()
            .map(|input| input.get_outpoint().get_tx_id().get_tx_id().expect("Not coinbase").get())
            .filter_map(|id| store.txs_by_id.contains_key(&id).then(|| id))
            .collect::<BTreeSet<_>>()
    }
}

fn unconfirmed_ancestors<T: GetParents>(tx: T, store: &MempoolStore) -> Ancestors {
    let mut visited = Ancestors(BTreeSet::new());
    tx.unconfirmed_ancestors_inner(&mut visited, store);
    visited
}

trait UnconfirmedAncestorsInner {
    fn unconfirmed_ancestors_inner(self, visited: &mut Ancestors, store: &MempoolStore);
}

impl<T: GetParents> UnconfirmedAncestorsInner for T {
    fn unconfirmed_ancestors_inner(self, visited: &mut Ancestors, store: &MempoolStore) {
        for parent in self.get_parents(store).iter() {
...
impl MempoolStore {
    ...
    fn contains_outpoint(&self, outpoint: &OutPoint) -> bool {
        matches!(self.txs_by_id.get(&outpoint.get_tx_id().get_tx_id().expect("Not Coinbase").get()),
    ...
    fn get_unconfirmed_outpoint_value(
        ...
        let tx_id = outpoint.get_tx_id().get_tx_id().expect("Not coinbase").clone();
    ...
    fn update_ancestor_count(&mut self, entry: &TxMempoolEntry) {
        for ancestor in unconfirmed_ancestors(entry, self).0 {
    ...
    M: GetMemoryUsage,
{
    ...
    pub(crate) fn get_update_min_fee_rate(&self) -> Result<FeeRate, TxValidationError> {
        ...
        // TODO test the actual fee decay. The unit tests don't yet mock the block production
        // mechanism so block_since_last_rolling_fee_bump is always false
...
mod tests {
    ...
    fn valued_outpoint(
        ...
        let outpoint_source_id = OutPointSourceId::Transaction(tx_id.to_owned());
    ...
    pub(crate) fn create_genesis_tx() -> Transaction {
        ...
        let outpoint_source_id = OutPointSourceId::Transaction(Id::new(&H256::zero()));
        let input = TxInput::new(outpoint_source_id, 0, genesis_message);
        ...
        fn confirmed_outpoints(&self) -> BTreeSet<ValuedOutPoint> {
            self.available_outpoints
                ...
                .map(|outpoint| {
                    ...
                    let index = outpoint.get_output_index();
                    let tx =
    ...
    */
    ...
    fn add_single_tx() -> anyhow::Result<()> {
        ...
        let relay_fee = Amount::from(get_relay_fee_from_tx_size(TX_SPEND_INPUT_SIZE));
    ...
    fn tx_duplicate_inputs() -> anyhow::Result<()> {
        ...
        let outpoint_source_id = OutPointSourceId::Transaction(genesis_tx.get_id());
        let input = TxInput::new(outpoint_source_id.clone(), 0, DUMMY_WITNESS_MSG.to_vec());
        let witness = b"attempted_double_spend".to_vec();
        let duplicate_input = TxInput::new(outpoint_source_id, 0, witness);
        let flags = 0;
        let locktime = 0;
    ...
    fn outpoint_not_found() -> anyhow::Result<()> {
        ...
        let outpoint_source_id = OutPointSourceId::Transaction(genesis_tx.get_id());
        let good_input = TxInput::new(outpoint_source_id.clone(), 0, DUMMY_WITNESS_MSG.to_vec());
        let flags = 0;
        let locktime = 0;
    ...
    fn tx_replace() -> anyhow::Result<()> {
        ...
        test_replace_tx(100.into(), replacement_fee)?;
        let res = test_replace_tx(100.into(), (relay_fee + 99).into());
        assert!(matches!(
            res,
            Err(Error::TxValidationError(
        ...
        let res = test_replace_tx(100.into(), 100.into());
        assert!(matches!(
            res,
            Err(Error::TxValidationError(
        ...
        let res = test_replace_tx(100.into(), 90.into());
        assert!(matches!(
            res,
            Err(Error::TxValidationError(
                TxValidationError::ReplacementFeeLowerThanOriginal { .. }
    ...
    fn tx_spend_input<T: GetTime, M: GetMemoryUsage>(
        ...
        let fee = fee.into().map_or_else(
            || Amount::from(get_relay_fee_from_tx_size(estimate_tx_size(1, 2))),
    ...
    fn one_ancestor_signal_is_enough() -> anyhow::Result<()> {
        ...
        let original_fee = Amount::from(200);
    ...
    fn tx_mempool_entry() -> anyhow::Result<()> {
        ...
        assert_eq!(unconfirmed_ancestors(entry1, &mempool.store).len(), 0);
        assert_eq!(unconfirmed_ancestors(entry2, &mempool.store).len(), 0);
        assert_eq!(unconfirmed_ancestors(entry3, &mempool.store).len(), 2);
        assert_eq!(unconfirmed_ancestors(entry4, &mempool.store).len(), 3);
        assert_eq!(unconfirmed_ancestors(entry5, &mempool.store).len(), 3);
        assert_eq!(unconfirmed_ancestors(entry6, &mempool.store).len(), 5);
    ...
    fn test_bip125_max_replacements<T: GetTime, M: GetMemoryUsage>(
        ...
        let fee = Amount::from(get_relay_fee_from_tx_size(TX_SPEND_INPUT_SIZE));
    ...
    fn spends_new_unconfirmed() -> anyhow::Result<()> {
        ...
        mempool.add_transaction(replaced_tx)?;
        let relay_fee = get_relay_fee_from_tx_size(TX_SPEND_INPUT_SIZE);
        let replacement_fee = Amount::from(100 + relay_fee);
    ...
    impl MockClock {
        ...
        fn increment(&self, inc: Time) {
            self.time.store(self.time.load(Ordering::SeqCst) + inc, Ordering::SeqCst)
        }
    }
    ...
    fn test_rolling_fee() -> anyhow::Result<()> {
        ...
        let child_2 = tx_spend_input(
            &mempool,
            TxInput::new(outpoint_source_id.clone(), 2, DUMMY_WITNESS_MSG.to_vec()),
            None,
            flags,
            locktime,
        )?;
        ...
        let child_2_high_fee = tx_spend_input(
            &mempool,
            TxInput::new(outpoint_source_id, 2, DUMMY_WITNESS_MSG.to_vec()),
        ...
        assert_eq!(
            ...
            (rolling_fee / FeeRate::new(2)).unwrap()
        );
