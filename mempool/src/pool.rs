...
impl TxMempoolEntry {
    fn new(tx: Transaction, pool: &MempoolStore) -> Self {
        let parents = tx
            .get_inputs()
            .iter()
    ...
    fn compare_fees(&self, other: &TxMempoolEntry) -> Ordering {
...
impl PartialOrd for TxMempoolEntry {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.compare_fees(other))
    }
}

impl Ord for TxMempoolEntry {
    fn cmp(&self, other: &Self) -> Ordering {
        self.compare_fees(other)
    }
}
...
struct MempoolStore {
    ...
    spender_txs: HashMap<Id<OutPoint>, Rc<TxMempoolEntry>>,
}
...
impl MempoolStore {
    ...
    fn add_tx(&mut self, tx: Transaction) -> Result<(), MempoolError> {
        ...
        let entry = Rc::new(TxMempoolEntry::new(tx, self));
        ...
        for outpoint in entry.tx.get_inputs().iter().map(|input| input.get_outpoint()) {
            self.spender_txs.insert(outpoint.get_id(), Rc::clone(&entry));
        }
        Ok(())
    }
    ...
    fn find_conflicting_tx(&self, tx: &OutPoint) -> Option<Rc<TxMempoolEntry>> {
        self.spender_txs.get(&tx.get_id()).cloned()
    }
}
...
where
    T: IntoIterator,
    T::Item: Idable<T::Item>,
    Id<T::Item>: Hash + Eq,
{
    let mut uniq = HashMap::new();
    iter.into_iter().any(move |x| uniq.insert(x.get_id(), x).is_some())
}
...
mod tests {
    ...
    fn valued_outpoint_with_id(
        tx_id: &Id<Transaction>,
        outpoint_index: u32,
        output: &TxOutput,
    ) -> (Id<OutPoint>, ValuedOutPoint) {
        let outpoint = OutPoint::new(tx_id.to_owned(), outpoint_index);
        let id = outpoint.get_id();
        let value = output.get_value();
        let valued_outpoint = ValuedOutPoint { outpoint, value };
        (id, valued_outpoint)
    }
    ...
    impl TxMempoolEntry {
        fn outpoints_created(&self) -> HashMap<Id<OutPoint>, ValuedOutPoint> {
            let id = self.tx.get_id();
            std::iter::repeat(id)
                .zip(self.tx.get_outputs().iter().enumerate())
                .map(|(id, (index, output))| valued_outpoint_with_id(&id, index as u32, output))
                .collect::<HashMap<_, _>>()
        }
    }

    impl MempoolStore {
        fn unconfirmed_outpoints(&self) -> HashMap<Id<OutPoint>, ValuedOutPoint> {
            self.txs_by_id
                .values()
                .cloned()
    ...
    impl MempoolImpl<ChainStateMock> {
        fn available_outpoints(&self) -> HashMap<Id<OutPoint>, ValuedOutPoint> {
            self.store
    ...
    pub(crate) struct ChainStateMock {
        ...
        outpoints: HashMap<Id<OutPoint>, OutPoint>,
    }
    ...
    impl ChainStateMock {
        pub(crate) fn new() -> Self {
            ...
            let outpoints = genesis_tx
                ...
                .enumerate()
                .map(|(index, _)| {
                    let outpoint = OutPoint::new(genesis_tx.get_id(), index as u32);
                    (outpoint.get_id(), outpoint)
                })
                .collect();
            Self {
        ...
        fn unspent_outpoints(&self) -> HashMap<Id<OutPoint>, ValuedOutPoint> {
            self.outpoints
                .values()
                .map(|outpoint| {
                    let outpoint_id = outpoint.get_id();
                    let value =
        ...
        fn confirmed_txs(&self) -> &HashMap<Id<Transaction>, Transaction> {
            &self.txs
        }
        ...
        fn confirmed_outpoints(&self) -> HashMap<Id<OutPoint>, ValuedOutPoint> {
            self.txs
                .values()
                .flat_map(|tx| {
                    std::iter::repeat(tx.get_id()).zip(tx.get_outputs().iter().enumerate()).map(
                        move |(tx_id, (i, output))| {
                            valued_outpoint_with_id(&tx_id, i as u32, output)
                        },
                    )
                })
                .collect()
    ...
    impl ChainState for ChainStateMock {
        fn contains_outpoint(&self, outpoint: &OutPoint) -> bool {
            self.outpoints.values().any(|value| *value == *outpoint)
        }
    }

    struct TxGenerator {
        coin_pool: HashMap<Id<OutPoint>, ValuedOutPoint>,
        num_inputs: usize,
        num_outputs: usize,
    }
    ...
    impl TxGenerator {
        fn new(
            ...
            let unconfirmed_outputs = HashMap::new();
        ...
        fn create_tx_generator(
            ...
            unconfirmed_outputs: &HashMap<Id<OutPoint>, ValuedOutPoint>,
            num_inputs: usize,
            num_outputs: usize,
        ) -> Self {
            let coin_pool = chain_state
                ...
                .map(|(id, valued_outpoint)| (id.clone(), valued_outpoint.clone()))
                .collect();
        ...
        fn generate_tx(&mut self) -> anyhow::Result<Transaction> {
            ...
            let spent_outpoints = inputs
                ...
                .collect::<HashMap<_, _>>();
            self.coin_pool.retain(|outpoint, _| {
                !spent_outpoints.values().any(|spent| spent.get_id() == *outpoint)
            });
            ...
            self.coin_pool.extend(
                std::iter::repeat(tx.get_id())
                    ...
                    .map(|(id, (i, output))| valued_outpoint_with_id(&id, i as u32, output)),
            );
            Ok(tx)
        }
    ...
    fn spend_input(
        ...
        let input_value =
            mempool.chain_state.get_outpoint_value(input.get_outpoint()).or_else(|_| {
                let outpoint_id = input.get_outpoint().get_id();
                mempool
                    .available_outpoints()
                    .get(&outpoint_id)
                    .cloned()
    ...
    fn tx_spend_input(
        ...
        let input_value = mempool
            .available_outpoints()
            .get(&input.get_outpoint().get_id())
            .map(|outpoint| outpoint.value)
            .ok_or_else(|| anyhow::anyhow!("unconfirmed output not found"))?
            .to_owned();
        let output_value = (input_value / 2.into()).expect("failed to divide input");
        let output_pay = TxOutput::new(output_value, Destination::PublicKey);
        let output_change_amount = (input_value - output_value).expect("underflow");
    ...
    fn one_ancestor_signal_is_enough() -> anyhow::Result<()> {
        ...
        let dummy_output = TxOutput::new(Amount::from(0), Destination::PublicKey);
