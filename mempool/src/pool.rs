...
const MAX_MEMPOOL_SIZE_BYTES: usize = 3_000_000;
...
mod tests {
    ...
    */
    ...
    fn rolling_fee() -> anyhow::Result<()> {
        ...
        let dummy_tx = TxGenerator::new().with_fee(Amount::from(100)).generate_tx(&mempool)?;
        log::debug!(
        ...
        assert_eq!(
            mempool.get_minimum_rolling_fee(),
            (rolling_fee / FeeRate::new(4)).unwrap()
        );
        log::debug!(
        ...
        mock_clock.increment(halflife);
        let another_dummy = TxGenerator::new().with_fee(Amount::from(100)).generate_tx(&mempool)?;
        mempool.add_transaction(another_dummy)?;
        assert_eq!(
            mempool.get_minimum_rolling_fee(),
            (rolling_fee / FeeRate::new(8)).unwrap()
        );

        mock_clock.increment(halflife);
        let final_dummy = TxGenerator::new().with_fee(Amount::from(100)).generate_tx(&mempool)?;
