    ...
    fn new<C: ChainState>(tx: Transaction, pool: &MempoolImpl<C>) -> Option<TxMempoolEntry> {
        let parents = tx
            ...
            .filter_map(|input| {
                pool.store
                    .txs_by_id
                    .get(&input.get_outpoint().get_tx_id().get_tx_id().expect("Not coinbase").get())
            })
            .cloned()
            .collect::<BTreeSet<_>>();
...
mod tests {
    ...
    fn tx_spend_input(
        ...
        let input_value = mempool
            ...
            .iter()
            .find_map(|valued_outpoint| {
                (valued_outpoint.outpoint == *input.get_outpoint()).then(|| valued_outpoint.value)
            })
            .ok_or_else(|| anyhow::anyhow!("No such unconfirmed output"))?;
        let output_value = (input_value / 2).expect("failed to divide input");
        let output_pay = TxOutput::new(output_value, Destination::PublicKey);
    ...
    fn one_ancestor_signal_is_enough() -> anyhow::Result<()> {
        ...
        let outpoint_source_id = OutPointSourceId::Transaction(tx.get_id());
        let replaceable_input =
            TxInput::new(outpoint_source_id.clone(), 0, DUMMY_WITNESS_MSG.to_vec());
        let other_input = TxInput::new(outpoint_source_id, 1, DUMMY_WITNESS_MSG.to_vec());
        ...
        let dummy_output = TxOutput::new(Amount::from_atoms(0), Destination::PublicKey);
