...
use prometheus::{
    core::{AtomicU64 as U64, Collector, GenericCounterVec},
    Error as PrometheusError, HistogramOpts, HistogramVec, Opts, Registry,
};

type CounterVecU64 = GenericCounterVec<U64>;

/// Metrics for RPC middleware
#[derive(Debug, Clone)]
pub struct RpcMetrics {
    requests_started: CounterVecU64,
    requests_finished: CounterVecU64,
    calls_time: HistogramVec,
    calls_started: CounterVecU64,
    calls_finished: CounterVecU64,
}
...
impl<M: Metadata> RequestMiddleware<M> for RpcMiddleware {
        ...
        X: Future<Output = Option<jsonrpc_core::Output>> + Send + 'static,
    {
        ...
        let metrics = self.metrics.clone();
        let transport_label = self.transport_label.clone();
        tracing::trace!(target: "rpc_metrics", "[{}] {} call: {:?}", transport_label, name, &call);
        if let Some(ref metrics) = metrics {
            metrics
                .calls_started
