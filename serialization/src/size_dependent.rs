...
//!
//! ```
//! # use serialization::{*, size_dependent::*};
//! # use hex_literal::hex;
//! #
...
use crate::{Compact, Decode, Encode, EncodeAsRef, Error, Input, Output};
use either::Either;
...
macro_rules! impl_FromSizeDependent {
    ...
    ($T:ident) => {
        impl ::core::convert::From<$crate::size_dependent::SizeDependent<$T>> for $T {
            fn from(wrapped: $crate::size_dependent::SizeDependent<$T>) -> $T {
                wrapped.0
            }
        }
    };
...
macro_rules! impl_SizedDecode_via_DecodeAll {
    // TODO support generic types
    ($T:ident) => {
        impl $crate::size_dependent::SizedDecode for $T where $T: $crate::Encode {
...
macro_rules! impl_SizedEncode_via_Encode {
    // TODO support generic types
    ($T:ident) => {
        impl $crate::size_dependent::SizedEncode for $T where $T: $crate::Decode {
            fn sized_using_encoded<R, F: FnOnce(&[u8]) -> R>(&self, f: F) -> R {
